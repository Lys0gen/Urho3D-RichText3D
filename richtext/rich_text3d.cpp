 ...
 , RichWidget(context)
{
    ...
    SubscribeToEvent(Urho3D::E_SCENEUPDATE, URHO3D_HANDLER(RichText3D, UpdateTickerAnimation));
}
...
{
    text_ = text;
    SetFlags(WidgetFlags_ContentChanged);
    CompileTextLayout();
    ResetTicker();
}
...
void RichText3D::ResetTicker()
{
    ...
    auto content_area = GetActualDrawArea();

    if (ticker_type_ == TickerType_Horizontal)
    {
        if (ticker_direction_ == TickerDirection_Negative)
            scroll_origin_ = Vector3(content_area.Right(), 0.0f, 0.0f);
        else
    ...
    {
        if (ticker_direction_ == TickerDirection_Negative)
            scroll_origin_ = Vector3(0.0f, GetActualDrawArea().Bottom(), 0.0f);
        else
...
{
    default_format_.font.face = face;
...
{
    return default_format_.font.size;
}
          ...
          if (bit->type == TextBlock::BlockType_Text) {
            ...
            if (new_block.is_visible) {
              ...
              if (new_block.image_height == 0) {
                ...
                // fit by width
                new_block.image_width = (float)(new_block.image_width > 0 ? new_block.image_width : clip_region_.Width());
  ...
  for (auto lit = lines_.Begin(); lit != lines_.End(); ++lit) {
    ...
    for (auto it = l->blocks.Begin(); it != l->blocks.End(); ++it) {
      if (it->type == TextBlock::BlockType_Image) {
        ...
        image_renderer->AddImage(Vector3((float)xoffset, (float)yoffset, 0.0f), it->image_width, it->image_height);
        ...
        text_renderer->AddText(it->text, Vector3((float)xoffset, (float)yoffset, 0.0f), it->format.color);
  ...
  if (ticker_type_ != TickerType_None)
  {
    ...
    if (ticker_type_ == TickerType_Horizontal)
    {
      if (clip_to_content_)
        ...
        ticker_max = (horizontal_dir == -1 ? content_size_.x_ : content_size_.x_ + clip_region_.Width());
    ...
    if (scrolled_out)
    {
      using namespace RichTextScrolledOut;
      VariantMap& eventData = GetEventDataMap();
      eventData[P_TEXT] = this;
      SendEvent(E_RICHTEXT_SCROLLED_OUT, eventData);
